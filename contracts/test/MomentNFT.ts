...
describe('MomentNFT', function () {
  ...
  async function deployMomentNFT() {
    ...
    const { result: momentNFTAddress } = await momentFactory.simulate.createBounty([
      maxSupply,
    ...
    const momentNFT = await hre.viem.getContractAt('MomentNFT', momentNFTAddress)
  ...
  describe('Deployment', function () {
    it('Should set the right params', async function () {
      const { momentNFT, maxSupply, totalRewardedTokens, admin } = await loadFixture(
        deployMomentNFT
      )
      ...
      expect(
        await momentNFT.read.hasRole([DEFAULT_ADMIN_ROLE, admin.account.address])
      ).to.equal(true)
    })
  })
  ...
  describe('Mints', function () {
    it('Should mint', async function () {
      const { momentNFT, rewardToken, totalRewardedTokens, admin } = await loadFixture(
        deployMomentNFT
      )
      const tokenId = 1n
      ...
      expect(await momentNFT.read.balanceOf([admin.account.address])).to.equal(1n)
      ...
      expect(await rewardToken.read.balanceOf([admin.account.address])).to.equal(
        totalRewardedTokens
      )
    })
  })
